        ...
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <button class="btn btn-success" data-toggle="modal" data-target="#createModal" style="width: 155px"
                    {{ user_has_permission(session('user')['userId'], 'shelf.create', 'disabled') }}>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <button type="button" class="btn btn-warning btn-edit mr-1"
                                    ...
                                    data-maxtiers="{{ $data->max_tiers }}"
                                    data-toggle="modal"
                                    data-target="#updateModal"
                                    {{ user_has_permission(session('user')['userId'], 'shelf.update', 'disabled') }}>
                                ...
                                <form class="d-inline" action="{{ route('pages.storageLocation.shelf.deActive') }}" method="POST">
                                    ...
                                    <button type="submit" class="btn btn-{{ $data->status_id == 1 ? 'danger' : 'success' }}"
                                        {{ user_has_permission(session('user')['userId'], 'shelf.deActive', 'disabled') }}>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.btn-edit', function() {
            ...
            modal.find('#update_max_tiers').val(button.data('maxtiers'));
        });
    });
</script>
