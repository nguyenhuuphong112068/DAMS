        ...
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <button class="btn btn-success" data-toggle="modal" data-target="#createModal" style="width: 155px"
                    {{ user_has_permission(session('user')['userId'], 'room.create', 'disabled') }}>
                    <i class="fas fa-plus"></i> Thêm mới Phòng
                </button>
            </div>

            <table id="data_table_room" class="table table-bordered table-striped w-100">
                <thead>
                    <tr>
                        <th>STT</th>
                        <th>Mã Phòng</th>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <button type="button" class="btn btn-warning btn-edit mr-1"
                                    ...
                                    {{ user_has_permission(session('user')['userId'], 'room.update', 'disabled') }}>
                                    <i class="fas fa-edit"></i>
                                </button>
                                <form class="d-inline" action="{{ route('pages.storageLocation.room.deActive') }}" method="POST">
                                    @csrf
                                    ...
                                    <button type="submit" class="btn btn-{{ $data->status_id == 1 ? 'danger' : 'success' }}"
                                        {{ user_has_permission(session('user')['userId'], 'room.deActive', 'disabled') }}>
