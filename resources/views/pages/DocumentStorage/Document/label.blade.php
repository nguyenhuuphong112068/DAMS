{{--
| TÀI LIỆU - IN NHÃN DÁN TÀI LIỆU
|
| Trang in độc lập, KHÔNG dùng layout.master. Mở tab mới khi bấm vào mã QR trên
| bảng tài liệu. Khung nhãn khai ở config/document.php, in vào giữa cuộn tem thực
| (media_*_mm) nên chừa lề trắng đều ~1mm. Cấu trúc và cách in giống nhãn lô vật tư
| của WMS.
|
| Bố cục: nửa trên là TÊN TÀI LIỆU (chữ to, canh giữa); nửa dưới chia đôi - trái là
| thông tin (vị trí, bộ phận, người tạo, ngày hết hạn), phải là MÃ QR ở góc dưới bên
...
| trên thanh công cụ (pages.DocumentStorage.shared.labelToolbar), mỗi lần in ghi audit
| log qua pages.documentStorage.document.labelPrinted.
--}}
...
@php
    ...
    // Dòng thông tin luôn giữ 1 dòng; cỡ chữ tự co theo độ dài để giá trị dài không bị
    // cắt cụt bằng "…". Ngày tháng luôn ngắn nên vẫn 2.2mm.
    ...
    $lblLines = [
        'Vị trí' => $document->location_code ?: ($document->location_name ?: '-'),
        'Bộ phận' => $document->department_name ?: '—',
        'Người tạo' => $document->owner ?: '—',
        'Hết hạn' => $lblDate($document->expired_date) ?: '—',
    ];
@endphp
<!DOCTYPE html>
<html lang="vi">
...
<head>
    <meta charset="utf-8">
    <title>Nhãn tài liệu - {{ $document->code }}</title>
...
<body>
    ...
    @include('pages.DocumentStorage.shared.labelToolbar', [
        'recordId' => $document->id,
        'logUrl' => route('pages.documentStorage.document.labelPrinted'),
    ...
    <div id="labelStack">
        <div class="label">
            ...
            <div class="bottom">
                <div class="info">
                    @foreach ($lblLines as $caption => $value)
                        <div class="line">
                            <span class="caption">{{ $caption }}</span>
                            <span class="value" style="font-size: {{ $lblValSize($value) }}mm">{{ $value }}</span>
                        </div>
                    @endforeach
                </div>
