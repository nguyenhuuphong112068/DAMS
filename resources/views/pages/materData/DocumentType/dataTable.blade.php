...
<div class="content-wrapper">
    <div class="card">
        <div class="card-header mt-4">
            <h3 class="card-title">Danh sách Loại Tài Liệu</h3>
        </div>
        <div class="card-body">
            <button class="btn btn-success btn-create mb-2" data-toggle="modal" data-target="#createModal" style="width: 155px"
                {{ user_has_permission(session('user')['userId'], 'documentType.create', 'disabled') }}>
                <i class="fas fa-plus"></i> Thêm mới
            </button>

            <table id="data_table_document_type" class="table table-bordered table-striped">
                        ...
                        <tr>
                            ...
                            <td class="text-center align-middle">
                                <button type="button" class="btn btn-warning btn-edit mb-1"
                                    ...
                                    {{ user_has_permission(session('user')['userId'], 'documentType.update', 'disabled') }}>
                                ...
                                <form class="form-deActive d-inline" action="{{ route('pages.materData.documentType.deActive') }}" method="POST">
                                    ...
                                    <button type="submit" class="btn btn-{{ $data->active ? 'danger' : 'success' }} btn-deactive-confirm"
                                        ...
                                        {{ user_has_permission(session('user')['userId'], 'documentType.deActive', 'disabled') }}>
...
<script>
    $(document).ready(function() {
        ...
        $('#data_table_document_type').DataTable({
            paging: true,
            lengthChange: true,
            searching: true,
            ordering: true,
